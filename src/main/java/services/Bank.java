...
public void setCurr(Employee curr) {
    ...
    public Boolean validate_pin(int pin)
    {
        ...
        curr_cust.setA(d.getAccount(curr_cust.getCnic()));
    ...
    public String transfer(String id,Double amount,int PIN)
    {
        ...
        if(st.equals("Success"))
        {
            ...
            d.update_balance(id, amount,date);
            d.update_balance(curr_cust.getA().getAcc_id(), -amount,date);
    ...
    public Boolean apply_for_loan(double loanAmount ,  Double loanDuration , Double interestRate  , String repayment)
    {
        Dbcontroller db = Dbcontroller.get();
        curr_cust.setA(db.getAccount(curr_cust.getCnic()));
    ...
    public void set_curr_loan(Integer req_id)
    {
        
        for (int i=0 ; i<ll.getL().size() ; i++)
        {
            if (ll.getL().get(i).getReqid()==req_id)
            {
    ...
    public void synccust()
    {
        ...
        curr_cust.setA(d.getAccount(get_curr_cust_cnic()));
    }
